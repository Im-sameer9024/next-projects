...
export const columns: ColumnDef<CourseColumn>[] = [
  {
    ...
    cell: ({ row }) => (
      <p className="leading-snug font-medium text-slate-800">{row.original.title}</p>
    ),
    sortingFn: (rowA, rowB) => {
      return rowA.original.title.toLowerCase().localeCompare(rowB.original.title.toLowerCase());
    },
  },

  {
    ...
    cell: ({ row }) => {
      ...
      return <p className="text-slate-600 tabular-nums">{price ? `₹${price}` : "Free"}</p>;
    },
    sortingFn: (rowA, rowB) => {
      // 👈 add this here
